...
def p_chart_limits(defect_series: pd.Series, subgroup_size: int = 10) -> Dict[str, float]:
    """P-chart limits treating malignant diagnosis as the 'defect' event."""
...
def six_sigma_metrics(df: pd.DataFrame) -> Dict[str, float]:
    """Six Sigma metrics treating malignant diagnosis as the non-conforming event."""
    malignant_count = int((df["diagnosis"] == 0).sum())
    opportunities = int(len(df))
    dpmo = (malignant_count / opportunities) * 1_000_000

    return {
        "malignant_count": malignant_count,
        "total_samples": opportunities,
        "DPMO": float(dpmo),
